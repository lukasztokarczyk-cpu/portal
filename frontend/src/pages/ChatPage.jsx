...
export default function ChatPage() {
  ...
  useEffect(() => {
    api.get('/weddings/my').then((res) => {
      setWedding(res.data);
      return api.get(`/messages/wedding/${res.data.id}`);
    }).then((res) => {
      setMessages(res.data.messages);
      if (res.data.unreadCount > 0) api.patch(`/messages/wedding/${res.data.id}/mark-read`);
    }).catch(console.error);
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      api.get(`/messages/wedding/${wedding.id}`).then((res) => {
        setMessages(res.data.messages);
        if (res.data.unreadCount > 0) api.patch(`/messages/wedding/${wedding.id}/mark-read`);
      });
    }, 10000);
  ...
  const sendMessage = async (e) => {
    ...
    try {
      const res = await api.post(`/messages/wedding/${wedding.id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <h1 className="text-2xl font-bold text-gray-800 mb-4">Wiadomości</h1>

      <div className="flex-1 overflow-y-auto space-y-3 mb-4 pr-1">
        {messages.length === 0 && <p className="text-center text-gray-400 py-8">Brak wiadomości. Napisz pierwszą!</p>}
        {messages.map((msg) => {
